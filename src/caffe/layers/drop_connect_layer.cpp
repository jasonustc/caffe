...
void DropConnectLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	threshold_ = this->layer_param_.drop_connect_param().dropout_ratio();
	DCHECK(threshold_ > 0.);
	DCHECK(threshold_ < 1.);
	scale_ = 1. / (1. - threshold_);
	uint_thres_ = static_cast<unsigned int>(UINT_MAX * threshold_);
	const int num_output = this->layer_param_.drop_connect_param().num_output();
	bias_term_ = this->layer_param_.drop_connect_param().bias_term();
	N_ = num_output;
	const int axis = bottom[0]->CanonicalAxisIndex(
		this->layer_param_.drop_connect_param().axis());
	...
	if (this->blobs_.size() > 0) {
	  ...
	  shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
		  this->layer_param_.drop_connect_param().weight_filler()));
	  ...
	  if (bias_term_) {
		  ...
		  shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
			  this->layer_param_.drop_connect_param().bias_filler()));
		  bias_filler->Fill(this->blobs_[1].get());
	  }
	  // Initialize the weight multiplier
	  this->weight_multiplier_.Reshape(bottom[0]->num(), bottom[0]->channels(),
		  weight_shape[0], weight_shape[1]);
	  //Initialize the weight after dropout
	  //one sample one channel corresponding to one time drop connect
	  this->dropped_weight_.Reshape(bottom[0]->num(), bottom[0]->channels(),
		  weight_shape[0], weight_shape[1]);
...
void DropConnectLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const int count = this->weight_multiplier_.count();
  //weight: K_xN_
