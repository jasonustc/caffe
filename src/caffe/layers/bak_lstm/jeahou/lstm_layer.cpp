...
/*
 * TODO: to get the end state of variant sequence length
 * currently, we can only deal with fixed length sequence
 */
template <typename Dtype>
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  for (int t = 1; t <= this->T_; ++t) {
	...
	// here if we only have two input blobs, eltwise sum is just 
	// like elementwise multiply
    {
