...
namespace caffe {
	...
	string PredRecurrentLayer<Dtype>::int_to_str(const int t) const {
		ostringstream num;
		num << t;
		return num.str();
	}
	...
	template <typename Dtype>
	void PredRecurrentLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		CHECK_GE(bottom[0]->num_axes(), 2)
			<< "bottom[0] must have at least 2 axes -- (#timesteps, #streams, ...)";
		//bottom[0]: h_0, bottom[1]: c_0, bottom[2]: cont, bottom[3}: static input x
		//timesteps
		T_ = bottom[0]->shape(0);
		//streams
		N_ = bottom[0]->shape(1);
		...
		//the cont indicator
		CHECK_EQ(bottom[2]->num_axes(), 2)
			<< "bottom[2] must have exactly 2 axes -- (#timesteps, #streams)";
		CHECK_EQ(T_, bottom[2]->shape(0));
		CHECK_EQ(N_, bottom[2]->shape(1));
		...
		//bottom[0] -> h_0
		net_param.add_input(recur_input_names[0]);
		...
		//bottom[1] -> c_0
		net_param.add_input(recur_input_names[1]);
		net_param.add_input_shape()->CopyFrom(input_shape);
		...
		h_input_blob_ = CHECK_NOTNULL(unrolled_net_->blob_by_name(recur_input_names[0]).get());
		c_input_blob_ = CHECK_NOTNULL(unrolled_net_->blob_by_name(recur_input_names[1]).get());
		...
		// We should have 3 inputs (c_0, h_0 and cont), plus a number of recurrent inputs,
		// plus maybe a static input.
		CHECK_EQ(3 + num_recur_blobs + static_input_,
	...
	template <typename Dtype>
	void PredRecurrentLayer<Dtype>::Reset() {
	...
	template <typename Dtype>
	void PredRecurrentLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		...
		//here we copy the c_0 and h_0 data from input, so don't initiated 
		//it from T data
...
//		}
	...
	template <typename Dtype>
	void PredRecurrentLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
	STUB_GPU_FORWARD(PredRecurrentLayer, Forward);
#endif

	INSTANTIATE_CLASS(PredRecurrentLayer);
