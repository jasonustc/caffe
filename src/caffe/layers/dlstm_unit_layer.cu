...
namespace caffe{
	...
	__global__ void DLSTMUnitForward(const int nthreads, const int dim,
		...
		CUDA_KERNEL_LOOP(index, nthreads){
			...
			const Dtype c = c_prev * f * flush[n] + i * g;
			C[index] = c;
	...
	__global__ void DLSTMActsBackward(const int nthreads, const int dim,
		...
		CUDA_KERNEL_LOOP(index, nthreads){
			...
			if (d < 3 * dim){
				X_diff[index] = X_act * (Dtype(1) - X_act) * X_acts_diff[index];
			}
			else{
	...
	void DLSTMUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		...
		DLSTMUnitBackward<Dtype> << < CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
			count, hidden_dim_, C_prev, X_acts, C, H, flush, C_diff, H_diff, C_prev_diff, X_acts_diff);
		CUDA_POST_KERNEL_CHECK;
		const int x_count = 4 * count;
		Dtype* X_diff = bottom[1]->mutable_gpu_data();
		DLSTMActsBackward<Dtype><<<CAFFE_GET_BLOCKS(x_count), CAFFE_CUDA_NUM_THREADS>>> (
