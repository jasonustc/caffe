...
namespace caffe{
	...
	void DLSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const{
		...
		//slice input X by to x_d_t
		//for decoder, the order is reversed
		...
		for (int dt = 1; dt <= this->T_; ++dt){
			...
			//Add layer to compute
			//   W_hc_h_{t-1} := W_hc * h_conted_{t-1}
			//we don't need to reset h in decoder LSTM, because all h
			//are connected to h_{t-1} or corresponding encoder h
			{
				...
				dw_param->add_top("W_hc_h_d_" + dtm1s);
				//sum along streams and times
			...
			//      gate_input_t := W_hc * h_conted_{t-1} + W_xc * x_t + b_c
			//                    = W_hc_h_{t-1} + W_xc_x_t + b_C
			//in the beginning of the decoding LSTM, we have no input, 
			//the decoder LSTM need to learn to reconstruct the whole sequence
			{
				if ((dt - 1) % sequence_len == 0){
					LayerParameter* dinput_sum_layer = net_param->add_layer();
					dinput_sum_layer->set_name("gate_input_d_" + dts);
					dinput_sum_layer->set_type("Copy");
					dinput_sum_layer->add_bottom("W_hc_h_d_" + dtm1s);
					dinput_sum_layer->add_top("gate_input_d_" + dts);
				}
				else{
					LayerParameter* dinput_sum_layer = net_param->add_layer();
			...
			//Add the LSTM Unit to learn sequence feature
			{
				...
				lstm_unit_param->add_bottom("gate_input_d_" + dts);
				...
				lstm_unit_param->set_name("unit_d_" + dts);
			}
//			output_concat_layer_mem.add_bottom(dec_h_name);
		...
		//add a silence layer to silence the not used h_dec_ts and c_dec_ts
		{
			...
			silence_param->set_name("silence_cs_hs");
			for (int i = 0; i < not_used_c_names.size(); i++){
//				silence_param->add_bottom(not_used_h_names[i]);
