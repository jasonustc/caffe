...
namespace caffe {
	...
	void LSTMUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		...
		this->decode_ = this->layer_param_.recurrent_param().decode();
	...
	void LSTMUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		//when begin a new sequence we need to flush(reset) the hidden states
		//if num_flush % sequence_length == 0, *flush = 0, else 1
		...
		for (int n = 0; n < num; ++n) {
			for (int d = 0; d < hidden_dim_; ++d) {
				...
				Dtype f;
				if (decode_){
					f = (*flush == 0) ? sigmoid(X[1 * hidden_dim_ + d]):
	...
	void LSTMUnitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		...
		for (int i = 0; i < top.size(); i++){
			ostringstream oss;
			oss << "top_" << i;
			top[i]->ToTxt(oss.str().c_str(), true);
		}
	}
