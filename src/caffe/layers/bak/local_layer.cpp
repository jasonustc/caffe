...
void LocalLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
	...
	E_.Reshape(1, 1, 1, K_);
	caffe_set<Dtype>(E_.count(), 1., E_.mutable_cpu_data());
}
	...
	for (int n = 0; n < num_; n++){
		...
		for (int m = 0; m < num_output_; m++){
			...
			caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, 1, N_, K_, (Dtype)1.,
				E_.cpu_data(), intermediate_.cpu_data(),
...
void LocalLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	Dtype* xt_data = intermediate_.mutable_cpu_data();
	//all the buffers are used after being asigned values,so just reshape is OK.
	for (int n = 0; n < num_; n++){
		//image 2 column
		im2col_cpu(bottom_data + bottom[0]->offset(n), channels_, height_,
			width_, kernel_size_, kernel_size_, pad_, pad_, stride_, stride_,
			...
			for (int m = 0; m < num_output_; m++){
				for (int k = 0; k < K_; k++){
					...
					caffe_cpu_axpby(N_, (Dtype)1.,
						intermediate_.cpu_data(), Dtype(1.0),
