...
namespace caffe{
	...
	void DLSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const{
		...
		CHECK_GT(num_rec_feature, 0) << "number of reconstruction feature dim must be positive";
		...
		vector<string> not_used_h_names;
		...
		for (int dt = 1; dt <= this->T_; ++dt){
			...
			int seq_reverse_t = dt % sequence_len == 0 ? dt - sequence_len + 1 :
				(dt / sequence_len) * sequence_len +
				(sequence_len - (dt % sequence_len) + 1);
			string seq_reversed_ts = this->int_to_str(seq_reverse_t);
...
//					not_used_h_names.push_back("h_" + dts);
//				}
//				else{
//					dw_param->add_bottom("h_" + dtm1s);
//				}
			...
			//                    = W_hc_h_{t-1} + W_xc_x_t + b_C
			{
				if ((dt - 1) % sequence_len == 0){
					...
					dinput_sum_layer->add_bottom("W_hc_h_" + dtm1s);
					//prediction here
					dinput_sum_layer->add_bottom("W_xc_x_d_" + otm1s);
					dinput_sum_layer->add_top("gate_input_" + dts);
				}
			}
...
//			for (int i = 0; i < not_used_h_names.size(); i++){
...
//					string rec_ts = dec_h_names[n * sequence_len + s - 1];
