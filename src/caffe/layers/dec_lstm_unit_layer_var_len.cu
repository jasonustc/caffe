...
__global__ void DLSTMUnitForward(const int nthreads, const int dim,
    const Dtype* C_prev, const Dtype* X, Dtype* C, Dtype* H, const Dtype* H_enc,
	const Dtype* C_enc, const Dtype* cont) {
  CUDA_KERNEL_LOOP(index, nthreads) {
...
void DLSTMUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* H_enc = bottom[2]->gpu_data();
  const Dtype* C_enc = bottom[3]->gpu_data();
  const Dtype* cont = bottom[4]->gpu_data();
  ...
  DLSTMUnitForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, hidden_dim_, C_prev, X_acts, C, H, H_enc, C_enc, cont);
  CUDA_POST_KERNEL_CHECK;
}
...
__global__ void DLSTMUnitBackward(const int nthreads, const int dim,
    ...
    Dtype* C_prev_diff, Dtype* X_diff, Dtype* H_enc_diff, Dtype* C_enc_diff,
	const Dtype* cont) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    const int n = index / dim;
    const int d = index % dim;
	if (cont[n] == 0){
		H_enc_diff[index] = H_diff[index];
		C_enc_diff[index] = C_diff[index];
	}
	else{
		const Dtype* X_offset = X + 4 * dim * n;
		const Dtype i = X_offset[d];
...
void DLSTMUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  //no flush is needed in decoder LSTM
  const int count = top[1]->count();
  const Dtype* C_prev = bottom[0]->gpu_data();
  const Dtype* X_acts = X_acts_.gpu_data();
  const Dtype* C = top[0]->gpu_data();
  const Dtype* H = top[1]->gpu_data();
  Dtype* H_enc_diff = bottom[2]->mutable_gpu_diff();
  Dtype* C_enc_diff = bottom[3]->mutable_gpu_diff();
  const Dtype* cont = bottom[4]->gpu_data();
