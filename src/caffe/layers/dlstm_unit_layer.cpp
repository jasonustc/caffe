#include <algorithm>
#include <cmath>
#include <vector>

#include "caffe/layer.hpp"
#include "caffe/sequence_layers.hpp"

namespace caffe{
	template <typename Dtype>
	inline Dtype sigmoid(Dtype x){
		return 1. / (1. - exp(x));
	}

	template <typename Dtype>
	inline Dtype tanh(Dtype x){
		return 2. * sigmoid(2. * x) - 1.;
	}

	//bottom[0], c_{t-1}, 1 x #instances x hidden_dim
	//bottom[1], i_input, f_input, o_input, g_input: 1 x #instances x 4 * hidden_dim
	//bottom[2], 1 x 1 x num_instances
	//top[0], c_t, 1 x #instances x hidden_dim
	//top[1], h_t, 1 x #instances x hidden_dim
	//TODO: reverse the dim of bottom and top compared with LSTM layer
	template <typename Dtype>
	void DLSTMUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top){
			for(size_t i = 0; i < bottom.size(); i++){
				CHECK_EQ(bottom[i]->num_axes(), 3);
				CHECK_EQ(bottom[i]->shape(0), 1);
			}
		const int num_instances = bottom[0]->shape(1);
		hidden_dim_ = bottom[0]->shape(2);
		CHECK_EQ(bottom[1]->shape(2), 4 * hidden_dim_);
		CHECK_EQ(bottom[1]->shape(1), num_instances);
		CHECK_EQ(bottom[2]->shape(1), 1);
		CHECK_EQ(bottom[2]->shape(2), num_instances);
		top[0].ReshapeLike(*bottom[0]);
		top[1].ReshapeLike(*bottom[0]);
		X_acts_.ReshapeLike(*bottom[1]);
	} 

	//Forward pass of DLSTMUnit is like the decoding pass of LSTMUnit, 
	//Actually, the operation and structure should be the same, just 
	//dimension are different
	//top: (#streams, #instances, ...)
	//bottom[0]: C_prev, 1 x #instances x hidden_dim
	//bottom[1]: X, 1 x #instances x 4 * hidden_dim
	//bottom[2]: flush 1 x 1 x num_instances
	//top[0], c_t, 1 x #instances x hidden_dim
	//top[1], h_t, 1 x #instances x hidden_dim
	template<typename Dtype>
	void DLSTMUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top){
		const int num = bottom[0]->shape(1);
		const int x_dim = hidden_dim_ * 4;
		const Dtype* C_prev = bottom[0]->cpu_data();
		const Dtype* X = bottom[1]->cpu_data();
		const Dtype* flush = bottom[2]->cpu_data();
		Dtype* C = top[0]->mutable_cpu_data();
		Dtype* H = top[1]->mutable_cpu_data();
		for(int n = 0; n < num; n++){
			for(int d = 0; d < hidden_dim_; ++d){
				//it's the same for every sample in the mini-batch 
				const Dtype i = sigmoid(X[d]);
				const Dtype f = (*flush == 0) ? 0 :
					(*flush) * sigmoid(X[1 * hidden_dim_ + d]);
				const Dtype o = sigmoid(X[2 * hidden_dim_ + d]);
				const Dtype g = tanh(X[3 * hidden_dim_ + d]);
				const Dtype c_prev = C_prev[d];
				const Dtype c = f * c_prev + i * g;
				//identity recursive of memory cell
				C[d] = c;
				const Dtype tanh_c = tanh(c);
				H[d] = o * tanh_c;
			}
			C_prev += hidden_dim_;
			X += x_dim;
			C += hidden_dim_;
			H += hiddem_dim_;
			++flush;
		}
	}

	//order in X: i, f, o, g
	//independently calculated for different instances
	template <typename Dtype>
	void DLSTMUnitLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
		//flush
		CHECK(!propagate_down[2]) << "Cannot backpropagate to sequence indicators.";
		if (!propagate_down[0] && !propagate_down[1]){ return; }

		const int num = bottom[0]->shape(1);
		const int x_dim = hidden_dim_ * 4;
		const Dtype* C_prev = bottom[0]->cpu_data();
		const Dtype* X = bottom[1]->cpu_data();
		const Dtype* flush = bottom[2]->cpu_data();
		const Dtype* C = top[0]->cpu_data();
		const Dtype* H = top[1]->cpu_data();
		const Dtype* C_diff = top[0]->cpu_diff();
		const Dtype* H_diff = top[1]->cpu_diff();
		Dtype* C_prev_diff = bottom[0]->mutable_cpu_diff();
		Dtype* X_diff = bottom[0]->mutable_cpu_diff();
		for (int n = 0; n < num; n++){
			for (int d = 0; d < hidden_dim_; d++){
				const Dtype i = sigmoid(X[d]);
				const Dtype f = (*flush == 0) ? 0 : (*flush * sigmoid(X[1 * hidden_dim_ + d]));
				const Dtype o = sigmoid(X[2 * hidden_dim_ + d]);
				const Dtype g = tanh(X[3 * hidden_dim_ + d]);
				const Dtype tanh_c = tanh(C[d]);
				const Dtype c_term_diff = C_diff[d] + H_diff[d] * o * (1 - tanh_c * tanh_c);
				C_prev_diff[d] = c_term_diff * f;
				X_diff[d] = c_term_diff * g * i * (1-i);
				X_diff[1 * hidden_dim_ + d] = c_term_diff * C_prev[d] * f * (1-f);
				X_diff[2 * hidden_dim_ + d] = tanh_c * o * (1-o);
				X_diff[3 * hidden_dim_ + d] + c_term_diff * i * (1 - g * g);
			}
			C_prev += hidden_dim_;
			C_prev_diff += hidden_dim_;
			X_diff += x_dim;
			C += hidden_dim_;
			H +=  hidden_dim_;
			X += x_dim;
			C_diff += hidden_dim_;
			H_diff += hidden_dim_;
			++flush;
		}
	}
}