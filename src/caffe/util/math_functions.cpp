...
//MxK to multiply KxN 2-D data
//C = alpha * op(A) * op(B) + beta * C
//TransA/TransB: row major or column major of A and B
//lda: columns of A
//ldb: columns of B
template<>
...
//added by xu shen
template <typename Dtype>
void caffe_rng_bernoulli_c(const int n, const Dtype p, Dtype* r) {
  CHECK_GE(n, 0);
  CHECK(r);
  CHECK_GE(p, 0);
  CHECK_LE(p, 1);
  ...
  boost::variate_generator<caffe::rng_t*, boost::bernoulli_distribution<Dtype> >
      variate_generator(caffe_rng(), random_distribution);
  for (int i = 0; i < n; ++i) {
    ((Dtype*)r)[i] = static_cast<Dtype>(variate_generator());
  }
}

template 
void caffe_rng_bernoulli_c<float>(const int n, const float p, float* r);

template 
void caffe_rng_bernoulli_c<double>(const int n, const double p, double* r);
